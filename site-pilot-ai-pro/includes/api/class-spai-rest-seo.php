...
class Spai_REST_SEO extends Spai_REST_API {
	...
	public function bulk_update( $request ) {
		...
		if ( empty( $updates ) || ! is_array( $updates ) ) {
			return $this->error_response( 'invalid_data', __( 'Updates array is required.', 'site-pilot-ai-pro' ), 400 );
		}
	...
	public function get_yoast_seo( $request ) {
		if ( ! $this->seo->is_yoast_active() ) {
			return $this->error_response( 'plugin_inactive', __( 'Yoast SEO is not active.', 'site-pilot-ai-pro' ), 400 );
		}
	...
	public function get_rankmath_seo( $request ) {
		if ( ! $this->seo->is_rankmath_active() ) {
			return $this->error_response( 'plugin_inactive', __( 'RankMath is not active.', 'site-pilot-ai-pro' ), 400 );
		}
	...
	public function get_aioseo_seo( $request ) {
		if ( ! $this->seo->is_aioseo_active() ) {
			return $this->error_response( 'plugin_inactive', __( 'AIOSEO is not active.', 'site-pilot-ai-pro' ), 400 );
		}
	...
	public function get_seopress_seo( $request ) {
		if ( ! $this->seo->is_seopress_active() ) {
			return $this->error_response( 'plugin_inactive', __( 'SEOPress is not active.', 'site-pilot-ai-pro' ), 400 );
		}
