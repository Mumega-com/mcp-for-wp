...
class Spai_REST_Themes extends Spai_REST_API {
	...
	public function update_settings( $request ) {
		...
		if ( empty( $settings ) ) {
			return $this->error_response(
				'missing_settings',
				__( 'Settings data is required.', 'site-pilot-ai-pro' ),
				400
			);
		}
	...
	public function get_settings_category( $request ) {
		...
		if ( ! isset( $settings[ $category ] ) ) {
			return $this->error_response(
				...
				sprintf( __( 'Settings category "%s" not found.', 'site-pilot-ai-pro' ), $category ),
				404
			);
		}
	...
	private function check_astra() {
		...
		if ( 'astra' !== $theme_info['slug'] ) {
			return new WP_Error(
				'not_astra',
				__( 'This endpoint requires Astra theme to be active.', 'site-pilot-ai-pro' )
			);
		}
	...
	public function update_astra_colors( $request ) {
		...
		if ( empty( $colors ) ) {
			return $this->error_response(
				'missing_colors',
				__( 'Color data is required.', 'site-pilot-ai-pro' ),
				400
			);
		}
	...
	public function update_astra_typography( $request ) {
		...
		if ( empty( $typography ) ) {
			return $this->error_response(
				'missing_typography',
				__( 'Typography data is required.', 'site-pilot-ai-pro' ),
				400
			);
		}
