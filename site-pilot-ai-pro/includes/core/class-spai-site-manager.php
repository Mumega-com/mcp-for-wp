...
class Spai_Site_Manager {
	...
	public function get_menu( $menu_id ) {
		...
		if ( ! $menu ) {
			return new WP_Error( 'not_found', __( 'Menu not found.', 'site-pilot-ai-pro' ) );
		}
	...
	public function create_menu( $data ) {
		...
		if ( empty( $name ) ) {
			return new WP_Error( 'missing_name', __( 'Menu name is required.', 'site-pilot-ai-pro' ) );
		}
	...
	public function delete_menu( $menu_id ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'delete_failed', __( 'Failed to delete menu.', 'site-pilot-ai-pro' ) );
		}
	...
	public function delete_menu_item( $item_id ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'delete_failed', __( 'Failed to delete menu item.', 'site-pilot-ai-pro' ) );
		}
	...
	public function get_global_styles() {
		if ( ! function_exists( 'wp_get_global_styles' ) ) {
			return new WP_Error( 'not_supported', __( 'Global styles require WordPress 5.9+ and a block theme.', 'site-pilot-ai-pro' ) );
		}
	...
	public function get_page_templates() {
		...
		$result = array(
			'default' => __( 'Default Template', 'site-pilot-ai-pro' ),
		);
		...
		if ( did_action( 'elementor/loaded' ) ) {
			$result['elementor_header_footer'] = __( 'Elementor Full Width', 'site-pilot-ai-pro' );
			$result['elementor_canvas'] = __( 'Elementor Canvas', 'site-pilot-ai-pro' );
		}
	...
	public function set_page_template( $page_id, $template ) {
		...
		if ( ! $page || 'page' !== $page->post_type ) {
			return new WP_Error( 'not_found', __( 'Page not found.', 'site-pilot-ai-pro' ) );
		}

		// Validate template.
		$valid_templates = array_keys( $this->get_page_templates() );
		if ( ! in_array( $template, $valid_templates, true ) && 'default' !== $template && '' !== $template ) {
			return new WP_Error( 'invalid_template', __( 'Invalid template.', 'site-pilot-ai-pro' ) );
		}
