...
class Spai_WooCommerce {
	...
	public function get_product( $id ) {
		if ( ! $this->is_active() ) {
			return new WP_Error( 'wc_not_active', __( 'WooCommerce is not active.', 'site-pilot-ai-pro' ), array( 'status' => 400 ) );
		}

		$product = wc_get_product( $id );
		if ( ! $product ) {
			return new WP_Error( 'not_found', __( 'Product not found.', 'site-pilot-ai-pro' ), array( 'status' => 404 ) );
		}
	...
	public function create_product( $data ) {
		...
		if ( ! $product_id ) {
			return new WP_Error( 'create_failed', __( 'Failed to create product.', 'site-pilot-ai-pro' ), array( 'status' => 500 ) );
		}
	...
	public function get_order( $id ) {
		...
		if ( ! $order ) {
			return new WP_Error( 'not_found', __( 'Order not found.', 'site-pilot-ai-pro' ), array( 'status' => 404 ) );
		}
	...
	public function get_customer( $id ) {
		...
		if ( ! $customer->get_id() ) {
			return new WP_Error( 'not_found', __( 'Customer not found.', 'site-pilot-ai-pro' ), array( 'status' => 404 ) );
		}
