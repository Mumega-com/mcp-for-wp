...
class Spai_Multilang {
	...
	public function create_post_translation( $post_id, $language, $data ) {
		if ( ! $this->is_active() ) {
			return new WP_Error( 'no_multilang', __( 'No multilingual plugin is active.', 'site-pilot-ai-pro' ), array( 'status' => 400 ) );
		}

		// Validate language.
		$languages = wp_list_pluck( $this->get_languages(), 'code' );
		if ( ! in_array( $language, $languages, true ) ) {
			return new WP_Error( 'invalid_language', __( 'Invalid language code.', 'site-pilot-ai-pro' ), array( 'status' => 400 ) );
		}

		// Check if translation already exists.
		$existing = $this->get_post_translations( $post_id );
		if ( isset( $existing[ $language ] ) ) {
			return new WP_Error( 'translation_exists', __( 'Translation already exists for this language.', 'site-pilot-ai-pro' ), array( 'status' => 409 ) );
		}
		...
		switch ( $this->detected_plugin ) {
			...
			case 'translatepress':
				return new WP_Error( 'not_supported', __( 'TranslatePress uses inline translation, not separate posts.', 'site-pilot-ai-pro' ), array( 'status' => 400 ) );
			default:
				return new WP_Error( 'unknown_plugin', __( 'Unknown multilingual plugin.', 'site-pilot-ai-pro' ), array( 'status' => 500 ) );
		}
	}
	...
	private function create_wpml_translation( $post_id, $language, $data ) {
		...
		if ( ! $original ) {
			return new WP_Error( 'not_found', __( 'Original post not found.', 'site-pilot-ai-pro' ), array( 'status' => 404 ) );
		}
