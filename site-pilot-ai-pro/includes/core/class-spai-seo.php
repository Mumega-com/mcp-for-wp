...
class Spai_SEO {
	...
	public function get_post_seo( $post_id, $plugin = null ) {
		...
		if ( ! $post ) {
			return new WP_Error( 'not_found', __( 'Post not found.', 'site-pilot-ai-pro' ) );
		}

		$plugin = $plugin ?: $this->get_active_plugin();

		if ( ! $plugin ) {
			return new WP_Error( 'no_seo_plugin', __( 'No SEO plugin is active.', 'site-pilot-ai-pro' ) );
		}
		...
		return new WP_Error( 'unsupported_plugin', __( 'SEO plugin not supported.', 'site-pilot-ai-pro' ) );
	}
	...
	public function analyze_post( $post_id ) {
		...
		if ( $title_len < 30 ) {
			$analysis['warnings'][] = __( 'SEO title is too short (under 30 characters).', 'site-pilot-ai-pro' );
		} elseif ( $title_len > 60 ) {
			$analysis['warnings'][] = __( 'SEO title is too long (over 60 characters).', 'site-pilot-ai-pro' );
		}

		// Check meta description.
		$desc = $seo_data['description'] ?? '';
		if ( empty( $desc ) ) {
			$analysis['issues'][] = __( 'Missing meta description.', 'site-pilot-ai-pro' );
		} else {
			$desc_len = strlen( $desc );
			if ( $desc_len < 120 ) {
				$analysis['warnings'][] = __( 'Meta description is too short (under 120 characters).', 'site-pilot-ai-pro' );
			} elseif ( $desc_len > 160 ) {
				$analysis['warnings'][] = __( 'Meta description is too long (over 160 characters).', 'site-pilot-ai-pro' );
			}
		}
		...
		if ( empty( $keyword ) ) {
			$analysis['suggestions'][] = __( 'Consider adding a focus keyword.', 'site-pilot-ai-pro' );
		} else {
			// Check if keyword is in title.
			if ( stripos( $title, $keyword ) === false ) {
				$analysis['warnings'][] = __( 'Focus keyword not found in SEO title.', 'site-pilot-ai-pro' );
			}
			// Check if keyword is in description.
			if ( ! empty( $desc ) && stripos( $desc, $keyword ) === false ) {
				$analysis['suggestions'][] = __( 'Consider adding focus keyword to meta description.', 'site-pilot-ai-pro' );
			}
		}
		...
		if ( $content_len < 300 ) {
			$analysis['warnings'][] = sprintf(
				...
				__( 'Content is short (%d words). Consider expanding to at least 300 words.', 'site-pilot-ai-pro' ),
				$content_len
			);
		}
